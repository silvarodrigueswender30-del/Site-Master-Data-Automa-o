...
export interface PricingGridProps {
  ...
  sectionBackgroundOverlay?: string; 
  animation?: AnimationType | AnimationConfig | false; 
  // Layout customization
  scaleRecommended?: boolean; 
...
const PriceDisplay: React.FC<PriceDisplayProps> = ({ price, className }) => {
  const textColor = className || 'text-card-foreground';
  const periodColor = className ? 'text-white/80' : 'text-gray-foreground';
      ...
      <div className="inline-flex rounded-lg border border-gray-200  p-1 shadow-sm">
        <button
          ...
          className={cn(
            ...
            billing === 'monthly'
              ...
              : 'bg-transparent text-card-foreground hover:bg-gray-50'
          )}
        >
          Monthly
        </button>
        <button
          ...
          className={cn(
            ...
            billing === 'annual'
              ? cn(activeColor, 'text-white shadow-sm')
              : 'bg-transparent text-card-background hover:bg-gray-50'
          )}
        >
...
const PricingTierCard: React.FC<PricingTierCardProps> = ({
  ...
  if (tier.cardColor) {
    ...
    buttonColor = tier.buttonColor || (needsLightButton ? ' hover:bg-gray-100' : colorClasses.buttonColor);
    buttonTextColor = tier.buttonTextColor || (needsLightButton ? 'text-card-foreground' : colorClasses.buttonTextColor);
    badgeColor = tier.badgeColor || (needsLightButton && isRecommended ? 'text-card-foreground' : colorClasses.badgeColor);
    ...
    badgeColor = tier.badgeColor || (needsLightButton && isRecommended ? 'text-card-foreground' : defaultBadgeColor);
    ...
    if (tier.buttonColor) {
      ...
      if (needsLightButton) {
        ...
        buttonTextColor = 'text-card-foreground';
      ...
      <div 
        className={cn(
          ...
          hasGlassmorphism && !tier.cardBackgroundImage && [
            'backdrop-blur-md /10 border-white/20',
        ...
        <div className="relative z-10 p-8 flex flex-col h-full">
          ...
          <div className="text-center pb-6">
            <Typography variant="caption" className={cn(
              ...
              (hasGlassmorphism || tier.cardBackgroundImage) ? 'text-white' : 'text-card-foreground'
            )}>
              {tier.name}
            </Typography>
            {tier.description && (
              <Typography className={cn(
                'text-base text-start text-card-foreground',
                (hasGlassmorphism || tier.cardBackgroundImage) ? 'text-white/90' : 'text-card-foreground'
              )}>
                {tier.description}
              </Typography>
            )}
              ...
              {tier.features.map((feature, idx) => {
                ...
                >
                  {feature.available !== false ? (
                    ...
                    <span className={cn(
                      'h-5 w-5 mt-0.5 flex-shrink-0',
                      hasImageBackground ? 'text-white/50' : 'text-card-foreground'
                    )}>×</span>
                  )}
                  <span className={cn(
                    'text-md font-medium leading-relaxed',
                    hasImageBackground ? 'text-white/90' : 'text-card-foreground'
                  )}>
                    {feature.label}
                  </span>
                </motion.li>
...
export const PricingGrid: React.FC<PricingGridProps> = ({
  ...
  const defaultCardBackgroundColor = '';
