    ...
    <div className={cn("flex flex-row gap-2 mb-6", alignmentClass, className)}>
      ...
      {companyName && (
        <div className={cn("mt-2 text-center")}>
          <Typography className={cn("font-semibold tracking-wide uppercase", companyTextClass)}>
            {companyName}
          </Typography>
        </div>
      )}
...
export const MaintenancePageHeading: React.FC<MaintenancePageHeadingProps> = React.memo(({
  ...
  if (children) {
    return (
      <div>
        <Typography className={cn("text-4xl sm:text-5xl lg:text-6xl font-bold mb-4 uppercase tracking-wide", alignmentClass, textColorClass, className)}>
          {children}
        </Typography>
      </div>
    );
  ...
  const AnimatedValue = React.useCallback(({ value, label }: { value: number; label: string }) => {
    ...
    return (
      ...
      >
        ...
        <Typography className={cn("text-4xl sm:text-5xl lg:text-6xl font-bold", countdownTextClass)}>
          {displayValue}
        </Typography>
      </motion.div>
    );
