...
| `thickness` | `number` | `4` | Border width in px for `default`; dot diameter in px for `dots-bounce`. Not used by `bars` |
| `color` | `string` | `'primary'` | Tailwind class applied to the spinner element. Raw CSS color values are not supported |
| `className` | `string` | `''` | Additional Tailwind classes applied to the outermost container element |