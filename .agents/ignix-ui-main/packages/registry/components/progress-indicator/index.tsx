...
/** Label position for circular progress. */
...
/**
 * Linear progress bar sub-component.
 * @internal
 */
...
/**
 * ProgressIndicator renders either a linear bar or circular ring with optional
 * percentage label, multiple animation variants, and flexible label positions.
 *
 ...
 * @example
 * ```tsx
 * <ProgressIndicator type="linear" value={72} showPercentage />
 * <ProgressIndicator type="circular" value={64} size={80} labelPosition="inside-center" />
 * ```
 */
export const ProgressIndicator = React.memo(function ProgressIndicator({
  ...
  const defaultFormatter = React.useCallback((value: number) => `${Math.round(value)}%`, []);
